...
public class Attack {
    ...
    public void pokemonSelect(int nowAttackPlayer,int playerbeAttack,ArrayList<Pokemon> playerattack,ArrayList<Pokemon> playerbeingattack){
        ...
        stopattack=false;
        exe=false;
        if(playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).remainHealth==0){
            for(int i=0;i<playerbeingattack.size();i++){//選擇被攻擊者寶可夢(生命值>=0)
                if(playerbeingattack.get(i).remainHealth>0){
                    player.get(playerbeAttack-1).nowPokemon=i;
                    System.out.println("系統已切換玩家(被攻擊者) "+ player.get(playerbeAttack-1).playername+" 的出戰寶可夢為編號 "+ (player.get(playerbeAttack-1).nowPokemon+1));
                    exe=true;
                    break;
            ...
            if(exe==false){
                System.out.println("玩家(被攻擊者) "+ player.get(nowAttackPlayer-1).playername+" 的所有寶可夢均已無生命值 ");
                stopattack=true;
            }
        }
    ...
    public boolean StopAttack(){
        ...
        if(exe==false){
            ...
            stopattack=true;
            if(stopattack==true){
                AfterAttackPrintInfo(player.get(1).playername,player.get(0).playername);
                dataRefresh();
                //stop.....
                return true;
            }else{
        ...
        if(exe==false){
            ...
            stopattack=true;
            if(stopattack==true){
                AfterAttackPrintInfo(player.get(0).playername,player.get(1).playername);
                dataRefresh();
                //stop.....
                return true;
            }else{
    ...
    public void normalattack(int nowAttackPlayer,int playerbeAttack,ArrayList<Pokemon> playerattack,ArrayList<Pokemon> playerbeingattack){//記得四捨五入
        //進行普通攻擊
        int causelife=(int)((playerattack.get(player.get(nowAttackPlayer-1).nowPokemon).attack+playerattack.get(player.get(nowAttackPlayer-1).nowPokemon).normalAttack));//造成傷害
        causelife=(int)Math.round((causelife*count(playerattack.get(player.get(nowAttackPlayer-1).nowPokemon).property, playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).property) * 1.0) / 1.0);
        if(playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).remainHealth>=causelife){
            playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).remainHealth-=causelife;
            playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).roundexperience+=causelife;
            playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).totalexperience+=causelife;
        }
        else{
            playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).roundexperience+=playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).remainHealth;
            playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).totalexperience+=playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).remainHealth;
            playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).remainHealth=0;
    ...
    public void skillattack(int nowAttackPlayer,int playerbeAttack,ArrayList<Pokemon> playerattack,ArrayList<Pokemon> playerbeingattack,int skillAttackvalue){//記得四捨五入
        //進行技能攻擊
        int causelife=(int)((playerattack.get(player.get(nowAttackPlayer-1).nowPokemon).attack+skillAttackvalue));//造成傷害
        causelife=(int)Math.round((causelife*count(playerattack.get(player.get(nowAttackPlayer-1).nowPokemon).property, playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).property) * 1.0) / 1.0);
        if(playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).remainHealth>=causelife){
            playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).remainHealth-=causelife;
            playerbeingattack.get(player.get(playerbeAttack-1).nowPokemon).roundexperience+=causelife;
    ...
    public void AfterAttackPrintInfo(String winner,String looser){
    ...
    public void runaway(ArrayList<Pokemon> playernum,int n){//逃跑
            ...
            System.out.println("玩家"+player.get(n-1).playername+"已逃跑");
    }
    
    public void toswitch(ArrayList<Pokemon> playernum,String pokemonnum, int nowAttackPlayer){//切換寶可夢
        if(player.get(nowAttackPlayer-1).nowPokemon==(Integer.parseInt(pokemonnum)-1)){
            ...
            if(playernum.get(Integer.parseInt(pokemonnum)-1).remainHealth==0){
                ...
                player.get(nowAttackPlayer-1).nowPokemon=Integer.parseInt(pokemonnum)-1;
                System.out.println("已將目前玩家"+player.get(nowAttackPlayer-1).playername+"的寶可夢更換為"+playernum.get(Integer.parseInt(pokemonnum)-1).property);
    ...
    public void superPotion(ArrayList<Pokemon> playernum, int nowAttackPlayer){//使用好傷藥
        if(player.get(nowAttackPlayer-1).superPotion==0){
            System.out.println("好傷藥(Super Potion)已使用完畢");
        }else{
            player.get(nowAttackPlayer-1).superPotion--;
            if((playernum.get(player.get(nowAttackPlayer-1).nowPokemon).remainHealth+50)<(playernum.get(player.get(nowAttackPlayer-1).nowPokemon).remainHealthlimit)){
                playernum.get(player.get(nowAttackPlayer-1).nowPokemon).remainHealth+=50;

            }else{
                playernum.get(player.get(nowAttackPlayer-1).nowPokemon).remainHealth=playernum.get(player.get(nowAttackPlayer-1).nowPokemon).remainHealthlimit;
            }
            System.out.println("----------------------------------------------------------");
            System.out.println("玩家"+player.get(nowAttackPlayer-1).playername+"已使用好傷藥(Super Potion)");
            System.out.println("剩餘次數:"+player.get(nowAttackPlayer-1).superPotion+"/2)");
            hw4.printInfo(player1, player2,player);
        }
    }
}
